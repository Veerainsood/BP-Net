...
import numpy as np
import cv2
from datasets.dataloaders import calibration_kitti
from skimage import io


from datasets.dataloaders.my_loader import depth2pointsrgb, depth2pointsrgbp
...
def depth_colorize(depth):
    ...
    depth = (255 * depth).astype('uint8') # H, W, C
    return cv2.applyColorMap(depth, cv2.COLORMAP_JET)

def feature_colorize(feature):
    feature = (feature - np.min(feature)) / ((np.max(feature) - np.min(feature)))

    u8 = (feature * 255).astype('uint8')
    
    bgr = cv2.applyColorMap(u8, cv2.COLORMAP_RAINBOW)
...
def save_mask_as_uint8colored(img, filename, colored=True, normalized=True):
    ...
    if(colored==True):
        img = (255 * img).astype('uint8') # H, W, C
